...
function clickResponse(e) {
  ...
  if (inputJSON.callback_id == "new_spare_request") {
    dialogBuilder("/newsparerequest",inputJSON.trigger_id);
    }
  
  if (inputJSON.callback_id == "fill_spare_request") {
    dialogBuilder("/fillsparerequest",inputJSON.trigger_id);
    }

  if (inputJSON.callback_id == "cancel_spare_request") {
    dialogBuilder("/cancelsparerequest",inputJSON.trigger_id);
    }
    
  if (inputJSON.callback_id == "list_requests") {
    dialogBuilder("/listsparerequests",inputJSON.trigger_id);
    }  
...
function requestSpare(userID,caller,league,date,drawTime) {
  ...
  sheet.appendRow([caller,league,requestID,date,drawTime,"Open","",userID]);
...
//Find the request that user is attempting to fill and mark as filled/fillsparerequested by user_name
function fillSpare(userID, caller, requestID) {
  var sheet = getSpareSheet("Spare Request Sheet");
  var rowNum = findRequest(sheet, requestID);
  var creatorID = sheet.getRange('H'+rowNum).getValue();
  ...
  var contactMessage = caller+" *has filled your spare request* "+requestID;
  Slack.sendPrivateMessage(creatorID, contactMessage, getProperty("API_TOKEN"));
  UrlFetchApp.fetch('https://slack.com/api/chat.postMessage',options);
}


//cancelsparerequest an existing request
...
//listsparerequests all open requests
...
function dialogBuilder(commandReceived, triggerID, userID) {
  ...
  if (commandReceived == "/newsparerequest") {
  ...
  if (commandReceived == "/fillsparerequest") {
  ...
  if (commandReceived == "/listsparerequests") {
  ...
  if (commandReceived == "/cancelsparerequest") {
    var requestID = listRequests("id");
